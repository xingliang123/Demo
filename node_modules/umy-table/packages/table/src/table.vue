<template>
  <div class="el-table"
    :class="[{
      'fixed-columns-roll-style': fixedColumnsRoll, // 修改源码, 导致固定列部分的横向滚动条不可拖动
      'el-table--fit': fit,
    ...
    <div v-if="fixedColumns.length > 0 && fixedColumnsRoll"
    ...
    <!--修改源码，解决合计时，固定列不可拖动，加上这个主要是解决修改层级导致阴影消失，为了给固定列加个阴影效果-->
    <div v-if="rightFixedColumns.length > 0 && fixedColumnsRoll"
...
<script>
  ...
  import { addResizeListener, removeResizeListener } from '../../../tools/utils/resize-event';
  import Mousewheel from '../../../tools/directives/mousewheel';
  import Locale from '../../../tools/mixins/locale';
  import Migrating from '../../../tools/mixins/migrating';
  ...
  export default {
    ...
    methods: {
      ...
      // 修改源码，切换选中状态,适用于选中很多，取消很多
      partRowSelections (datas, state) {
        this.store.partRowSelections(datas, state);
        this.store.updateAllSelected();
      },
