...
import merge from 'umy-table/tools/utils/merge';
    ...
    partRowSelections (datas, flas) {
      ...
      if (flas) {
          ...
          if (states.selectable) {
              ...
              let newData = datas.filter((items,indexs) => states.selectable.call(null, items, indexs))
              states.selection = this.removalDataObj([...newData, ...states.selection], rowKey)
          } else {
              states.selection = this.removalDataObj([...datas, ...states.selection], rowKey)
          }
      } else {
          if (states.selectable) {
              // 找出不是禁用的行，然后全选上直接复制给selection，避开循环赋值
              ...
              states.selection = states.selection.filter((x) => !newData.some((item) => x[rowKey] === item[rowKey]));
          } else {
             states.selection = states.selection.filter((x) => !datas.some((item) => x[rowKey] === item[rowKey]));
          }
      }
