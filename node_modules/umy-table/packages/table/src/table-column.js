          ...
          if (plTreeTable.rowId && column.treeNode && plTreeTable.useVirtual) {
              return (<div { ...props }>
                     <div class="cell--tree-node"
                         ...
                         ? <div class="pltree-loading"><i class={[iconLoaded]}></i></div>
                         : (row[plTreeTable.treeOpts.children] && row[plTreeTable.treeOpts.children].length > 0) ||
                          (plTreeTable.treeOpts.lazy && row[plTreeTable.treeOpts.hasChildren])
                               ? <div class={[{'tree--btn-wrapper-show': row.pl_table_expand && iconshow}, 'tree--btn-wrapper']}
                         ...
                         <div class="pl-tree-cell">{prefix}{children}</div>
                     </div>
                 </div>)
          } else {
            // 原始的el-table处理单元格信息
            return (<div { ...props } title={[showEllipsis === 'title' && children ? children : '']}>
                { prefix }
                { children }
            </div>)
          }
