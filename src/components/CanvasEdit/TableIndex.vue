<template>
  <div class="editbox">
    ...
    <div class="editor-table" ref="canvas-table" id="canvas-table"></div>
  ...
  export default {
    ...
    methods: {
      setTabel(tableHeadr, tableName) {
        ...
        this.$nextTick(() => {
          let arr = []
          tableHeadr.forEach(element => {
            arr.push({
              zdmc: element.jghbgLmc,
              zd: element.jghbgLbm
            })
          });
          instanceExport.command.executeInsertTable(1, tableHeadr.length, arr)
        })
        //recoveryTableFun()
      },
      //获取编辑器内容
