...
export function formatElementList(
  ...
  while (i < elementList.length) {
    ...
    if (el.type === ElementType.TITLE) {
      ...
      const { prefix, postfix, value, placeholder, code, type, valueSets, labelName, labelZd } =
        el.control!
      const {
...
export function zipElementList(payload: IElement[]): IElement[] {
  ...
  while (e < elementList.length) {
    ...
    if (element.titleId && element.level) {
      ...
      let copyData = JSON.parse(JSON.stringify(element))
      ...
      if (dateElement.valueList.length > 0) {
        dateElement.valueList.forEach((element: any) => {
          element.labelName = copyData.labelName
          element.labelZd = copyData.labelZd
        });
      }
      element = dateElement
