...
export class ContextMenu {
  ...
  private _render(payload: IRenderPayload): HTMLDivElement {
    ...
    let height1 = 0, left1 = 0
    if (!!this.container.offsetTop && this.container.offsetTop != 90) {
      height1 = 50
      left1 = 150
    }
    // 直接子菜单
    ...
    const adjustLeft =
      left + contextMenuWidth > innerWidth ? left - contextMenuWidth - left1 : left - left1
    contextMenuContainer.style.left = `${adjustLeft}px`
    // 下侧空间不足时，以菜单底部作为起始点
    const innerHeight = window.innerHeight
    const contextMenuHeight = contextmenuRect.height
    const adjustTop =
      top + contextMenuHeight > innerHeight ? top - contextMenuHeight - height1 : top - height1
