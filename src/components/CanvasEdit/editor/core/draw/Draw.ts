...
export class Draw {
  ...
  public insertElementList(payload: IElement[]) {
    ...
    let arr = []
    if (payload[0].control?.labelName != '') {
      let labelName: any;
      labelName = payload[0]?.control?.labelName
      for (var i = 0; i < labelName?.length; i++) {
        arr.push({ value: labelName[i], size: 16 })
      }
    }
    if (arr.length > 0) {
      arr.push({ value: '：', size: 16 })
      payload = [...arr, ...payload]
    }
    ...
    let labelNameArr: { style: IElement; value: any; labelZd: any }[] = [];

    if (!!payload[0].dateFormat && !!payload[0].labelName) {
      let nameArr = payload[0].labelName.split("")
      nameArr.forEach((item: any) => {
        labelNameArr.push({
          style: payload[0],
          value: item,
          labelZd: payload[0].labelZd
        })
      })
    }
    if (labelNameArr.length > 0) {
      payload = [...labelNameArr, ...payload]
    }
    let curIndex = -1
    // 判断是否在控件内
