...
export class CommandAdapt {
  ...
  public async paste() {
    const isReadonly = this.draw.isReadonly()
    if (isReadonly) return
    // let workerNav = navigator as any; // 使用 any 类型断言
  ...
  public insertTable(row: number, col: number, header: any, tableName: any, tableBm: any) {
    const isReadonly = this.draw.isReadonly()
    if (isReadonly) return
    const activeControl = this.control.getActiveControl()
    if (activeControl) return
    ...
    if (startIndex == -1 && endIndex == -1) {
      startIndex = this.draw.getElementList().length;
      endIndex = this.draw.getElementList().length
    }
    if (!~startIndex && !~endIndex) return
    const elementList = this.draw.getElementList()
    let offsetX = 0
    ...
    for (let c = 0; c < col; c++) {
      colgroup.push({
        width: colWidth,
        zdmc: header.length > 0 ? header[c].zdmc : '',
        zd: header.length > 0 ? header[c].zd : '',
        tableName: header.length > 0 ? header[c].tableMc : '',
        tableBm: header.length > 0 ? header[c].tableBm : '',
      })
    }
    // trlist
    const trList: ITr[] = []
    for (let r = 0; r < row; r++) {
      ...
      for (let c = 0; c < col; c++) {
        // let valueArr = []
        // valueArr = arr[c].zdmc.split("")
        // let 
        tdList.push({
          colspan: 1,
          rowspan: 1,
          value: [{ value: r == 0 && header.length > 0 ? header[c].zdmc : ZERO, size: 16 }]
        })
      }
      trList.push(tr)
    }
