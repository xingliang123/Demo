...
let containerExport = null;
let instanceExport = {};
let recoveryTableFun = null
export function windowOnload(editId: string) {

  const isApple =
    typeof navigator !== 'undefined' && /Mac OS X/.test(navigator.userAgent)

  // 1. 初始化编辑器
  const container = document.querySelector<HTMLDivElement>('.canvas' + editId)!
  ...
  recoveryTableFun = recoveryTable()
      ...
      case ControlType.TEXT:
        new Dialog({
          ...
          onConfirm: payload => {
            ...
            const labelName = payload.find(p => p.name === 'zdMc')?.value
            const labelZd = payload.find(p => p.name === 'zd')?.value
  ...
  const handleContentChange = async function () {
    ...
    console.log(instance.command.getHTML(), instance.command.getText(), instance.command.getValue())
  ...
  sessionStorage.setItem("canvas" + editId, JSON.stringify(instance))
}
export {
  containerExport, recoveryTableFun, instanceExport
}